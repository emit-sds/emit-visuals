...
def main():
    ...
    #output_img = np.zeros((3, revisits.shape[1], revisits.shape[2]))
    subset = revisits > 0

    #output_img[:, subset] = np.array([255,255,255])[:,np.newaxis]

    #scaled_revisit = (revisits > 0).astype(np.float32)
    #scaled_revisit = revisits / args.max_val
    #scaled_revisit[scaled_revisit > 1] = 1

    output_img = np.zeros((subset.shape[0],subset.shape[1],3))
    output_img[subset,:] = 255
