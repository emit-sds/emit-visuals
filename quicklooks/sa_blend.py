...
def main():
    ...
    parser.add_argument('--mineral_subset', type=str, default=None, nargs='+')
...
#    }
    ...
    #'dolomite' :        [1, 255, 1],  # classic green
    #'chlorite' :        [115, 157, 98],  # very light green - pale green
    #'vermiculite' :     [189, 120, 66],  # brown
    #'illite+muscovite': [7, 250, 163],  #lighter blue / green
    #'montmorillonite' : [5, 173, 250],  #light blue 
    ...
    if len(args.mineral_subset) > 0:
        sa = sa[np.array([band_names.index(x) for x in args.mineral_subset]),...]
        band_names = args.mineral_subset
    ...
    for val in range(sa.shape[0]):
        ...
        if np.sum(sa_subset) > 0:

            sa_subset = ((sa_subset - np.percentile(sa_subset,5)) / np.percentile(sa_subset,95))
            output_img[:,subset] += np.squeeze(sa_subset) * np.array(colorlist[band_names[val]])[:,np.newaxis]
    
    del sa

    output_img = np.round(output_img*2)
    ...
    plt.legend(handles=[Patch(facecolor=np.array(colorlist[x])/255., edgecolor='black', label=x.capitalize().replace('e+m','e & M')) for x in colorlist.keys()], loc='lower left', fontsize=6)
